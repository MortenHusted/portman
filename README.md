...
**Environments are hermetic.** A service's env is a minimal base (fixed PATH, HOME, USER, TMPDIR) plus only the declared sources. Nothing is inherited from the daemon, your shell, mise, or ambient `.env` files. Composition is deterministic: base, then `env_files` in order, then secrets providers, then inline `env`, later wins. A missing `env_file` is a config error: the service lands in `failed` without retrying, until the next `portman up`. A service whose dependency has failed shows `failed` too, naming the dependency, and starts on its own once that dependency comes back. A secrets-provider outage retries under the restart policy if it looks transient, or fails the service on auth rejection, unless `secrets_optional` opts into the env-files-only fallback (flagged in `portman status`).
